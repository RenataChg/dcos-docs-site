...
- Marathon provides a `health` member of the task resource via the [REST API](/mesosphere/dcos/1.14/deploying-services/marathon-api/) that you can add to your service definition.
...
![GUI health check](/mesosphere/dcos/1.14/img/health-check-gui.png)

Figure 1. Web interface health check

## Command health check performance

When deploying a service which makes use of command health checks, the resources consumed by the health check itself must be considered. If the command executed by the health check requires significant CPU or memory to run, those resources should be added to the resource requirements of the task. In cases where a task only specifies a very small amount of CPU (0.1 or less), even a trivial health check will have trouble running reliably, so CPU greater than this should be specified on any task making use of a command health check.
